...
void display_length(struct trienode *curr,int count)//same as display functoin but displays the word only if it satisfies the count property
  {
    int i,j;
    for(i=0;i<255;i++)
    {
...
struct trienode *getnode()
  ...
  void display(struct trienode *curr)
  {
    ...
    for(i=0;i<255;i++)
    {
      if(curr->child[i]!=NULL)
      {
         word[length++]=i;//converts to char and stores in char array
